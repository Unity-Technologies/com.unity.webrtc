...
extern void *_libnvcuvid_so_1_tramp_table[];

// Can be sped up by manually parsing library symtab...
void _libnvcuvid_so_1_tramp_resolve(int i) {
  ...
  _libnvcuvid_so_1_tramp_table[i] = dlsym(h, sym_names[i]);
  CHECK(_libnvcuvid_so_1_tramp_table[i], "failed to resolve symbol '%s'", sym_names[i]);
}

// Helper for user to resolve all symbols
void _libnvcuvid_so_1_tramp_resolve_all(void) {
  size_t i;
  for(i = 0; i + 1 < sizeof(sym_names) / sizeof(sym_names[0]); ++i)
    _libnvcuvid_so_1_tramp_resolve(i);
}
