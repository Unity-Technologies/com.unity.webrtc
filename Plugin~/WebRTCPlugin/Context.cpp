...
#pragma warning(pop)
    ...
    void Context::ProcessAudioTrackSinkAdapter(AudioTrackSinkAdapter* sink, float* data, int32 size, int channels, int sampleRate)
    {
        if (sink)
            sink->ProcessAudio(data, size, channels, sampleRate);
    }
