...
namespace webrtc
{
    ...
    bool
    D3D12GraphicsDevice::ConvertI420Buffer(const ITexture2D* baseTex, rtc::scoped_refptr<webrtc::I420Buffer> buffer)
    {
        const D3D12Texture2D* tex = reinterpret_cast<const D3D12Texture2D*>(baseTex);
        assert(nullptr != tex);
        if (nullptr == tex)
            return false;
